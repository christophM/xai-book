...
Input: Trained model $\hat{f}$, data $X$ where each row refers to an observations denoted by $x^{(i)}$, target vector $y$, loss function $L$.

1. Estimate the original model error $e^{\text{orig}} = \frac 1 n \sum_{i = 1}^n L(y^{(i)}, \hat{f}(x^{(i)}))$  (e.g. mean squared error)
2. For each feature $j = 1,...,p$ do:
    - Generate feature matrix $X^{\text{perm}}$ by permuting feature $j$ in the data $X$. This breaks the association between feature $j$ and true outcome $y$. Each row of $X^{\text{perm}}$ contains replaced feature values for feature $j$ and is denoted by $x_{\text{perm}}^{(i)}$.
    - Estimate error $e^{\text{perm}} = \frac 1 n \sum_{i = 1}^n L(y^{(i)}, \hat{f}(x_{\text{perm}}^{(i)}))$ based on the predictions of the permuted data.
